...
- `npm i typescript @types/node tsx tsup -D` = instala TS, tipagem do node, 
  utilitário de execução de arquivos ts e o tsup para geração de builds em JS a partir do TS;
...
- .npmrc (save-exact=true) -> config para versão de dependencia instaladas fique fixa no package.json
  ...
  - Usando isso podemos usar bots de github como renovate que é um bot para tentar atualizar algumas
    dependecias especificas, ele atualiza, roda os testes automaticamente, e caso os testes passem,
    ele cria uma pull request para nós atualizarmos;
- `npm i dotenv`
- `npm i zod`
...
- `"start:dev": "tsx watch src/server.ts"` = script para compilar o codigo js para ts
   em produção e ficar vigiando mudanças; (DEV)
- `"build": "tsup src --out-dir build"` = script para gerar build do codigo ts para 
  js numa pasta "build" (PRODUÇÃO)
- `"start": "node build/server.js"` = script para iniciar o código diretamente com
   node na pasta build (PRODUÇÃO)
...
- Adicione um script para formatar lá no package.json: 
...
- uuid() = mais segurança para gerar rotas impossível de conseguir acessar outro 
  recurso mudando o id na url
- `npx prisma generate`: gera os tipos para o TS entender que existe as tabelas 
  do prisma. Ele gera um tipo "User" referente à sua tabela User; além disso, gera
  os metodos findUnique, etc;
- `npm i @prisma/client`: dependeica de produção, usada para ACESSAR o banco (prismaClient)

## Docker

- Criar um ambiente com dados de forma que, se não precisar mais dele, você simplesmente
  mata esse ambiente;
...
- E pronto, você não tem seu computador pessoal contaminado com dependencias de banco,
  ferramentas, etc;
...
- Adicione esses dois scripts ao package.json: `"test": "vitest run", "test:watch": "vitest",`
  - O vitest run apenas executa os testes e para. O vitest sem run fica observando
    mudanças e dispara os testes a cada nova mudança
...
- Ao rodar vai criar uma pasta /coverage/ que vai ter um index.html
  - Ao abrir esse index.html, você vai ver um gráfico de quanto % os arquivos foram
    cobertos. Foque nos que você está testando;
...
- PrismaUsersRep é um repositório que contém a regra da regra de negócio. Ele faz as op.
  ...
  por outro caso necessário; poderia ser SequelizeUsersRep, etc;
- UsersRep é apenas o arquivo que tipa o PrismaUsersRep ou qualquer outro arquivo que 
  concentra regras para falar com uma certa ferramenta como knex.js, TypeORM, Sequelize, etc;
- Conceitos usados até o momento: Inversão de Dependência (SOLID); Factorys (Creation Design Pattern) 

## TDD -> Test Driven Development

- Se você desenvolve o teste de uma regra de negócio, antes de implementar o teste
  ajuda a validar se a implementação funciona ou não;
- Etapas:
  - Red State: erro not teste, vemos que o app não funciona já como esperado; faça
...
- **JWT**: envia credenciais apenas no /login, backend valida se credenciais são válidas e, se
  ...
  - composição:
    - header/cabeçalho: qual algoritmo foi usado para criar o token (hs56, rs256, rsa256,
      que podem inclusive usar até duas chaves);
    - payload: qualquer informação desejada pelo usuário. Ex: sub(id), name, email...;
    - sign/assinatura: o que o backend gera de único. Pode até haver mudanças no paysload
      (de id de usuário por ex), mas o backend vai validar se o sign foi ele que gerou
      inicialmente e vai barrar;
...
- Um teste não pode interferir no outro (fazer fatias isoladas para testes, switch
  de testes, algo assim)
...
- Depois, no vite.config.ts, você configura quais arquivos vão usar o referido ambiente
  de edesenvolvimento (no caso, o nome dele é prisma)
- vá na pasta vitest-environment-prisma e dê um `npm link` para cirar um rep local;
- Depois volta na pasta geral do projeto e da um `npm link vitest-environment-prisma` para
  você conseguir rodar a configuração de ambientes que você fez rodar globalmente;
...
- Temos que configurar um pretest tambem para executar antes dos testes
  ...
  - logo:
    ...
    "test:install-prisma-environment": "npm link vitest-environment-prisma"}
      ...
      - o create prisma environment faz o link com o pacote da pasta prisma (como se tivesse
        feito o processo de entrar na pasta e dar npm link)
      - o install executa equivalente ao `npm link vitest-environment-prisma` para instalar
        o pacote globalmente;

## Segmentando BD para testes
